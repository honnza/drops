...
fn day1(part: char, s: &str) -> String {
    let nums: Vec<u32> = s.lines().map(|l| l.parse()).collect::<Result<_,_>>().unwrap();
    if part == 'a' {
        for x in &nums {
            for y in &nums {
                if x + y == 2020 {return (x * y).to_string()}
            }
        }
    } else {
        for x in &nums {
            for y in &nums {
                for z in &nums {
                    if x + y + z == 2020 {return (x * y * z).to_string()}
                }
            }
        }
    }
...
fn day2(part: char, s: &str) -> String {
    s.lines().filter(|line| {
        let (patt, pass) = str_split_once(line, ": ").unwrap();
        let (range, patt_c_str) = str_split_once(patt, " ").unwrap();
        let (min, max) = str_split_once(range, "-").unwrap();
        let patt_c: char = patt_c_str.parse().unwrap();
        if part == 'a' {
            let count: usize = pass.chars().filter(|pass_c|
                *pass_c == patt_c
            ).count();
            count >= min.parse().unwrap() && count <= max.parse().unwrap()
        } else {
            let minth_char = pass.chars().nth(min.parse::<usize>().unwrap() - 1).unwrap();
            let maxth_char = pass.chars().nth(max.parse::<usize>().unwrap() - 1).unwrap();
            (minth_char == patt_c) ^ (maxth_char == patt_c)
        }
    }).count().to_string()
}
